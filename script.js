...
for (let i = 0; i < dropList.length; i++) {
  for (let currency_code in country_list) {
    let selected = (i == 0 && currency_code == "USD") ? "selected" : (i == 1 && currency_code == "INR") ? "selected" : "";
...
function loadFlag(element) {
  ...
  if (!imgTag) {
    ...
    element.parentElement.appendChild(imgTag);
  }
...
function getExchangeRate() {
  ...
  const apiKey = 'e8e0ec79ee34690656902c54'; // Replace with your actual API key
  ...
  fetch(url)
    ...
    .catch(error => {
      console.error("Error fetching exchange rate:", error); // For debugging
